...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <Placemark>
            ...
            <description>
                <xsl:choose>
                    <xsl:when test="@corresp">
                        <xsl:value-of
                            select="$places[@xml:id = substring(current()/@corresp, 2)]//placeName[@type = 'full']"
                        />
                    </xsl:when>
                    <xsl:otherwise>
            ...
            <Style>
                <IconStyle>
                    <Icon>
                        <xsl:if test="@cert">
                            <href>http://www.obdurodon.org/~heb43/images/poet-dub.png</href>
                        </xsl:if>
                ...
                <coordinates>
                    <xsl:choose>
                        <xsl:when test="@corresp">
                            <xsl:value-of
                                select="
                                    $places[@xml:id = substring(current()/@corresp, 2)]
                ...
                <coordinates>
                    <xsl:choose>
                        <xsl:when test="..//affiliation[@key = 'origem'][1]/@corresp">
                            <xsl:value-of
                                select="
                                    $places[@xml:id = substring(current()/..//affiliation[@key = 'origem'][1]/@corresp, 2)]
                                    /location/concat(string-join(
                                    (substring-after(geo, ','),
                                    substring-before(geo, ','))
                                    , ','), ',100')"
                        ...
                        <xsl:otherwise>
                            <xsl:value-of
                                select="
                                    ..//affiliation[@key = 'origem'][1]/concat(string-join(
                                    (substring-after(geo, ','),
                                    substring-before(geo, ','))
                                    , ','), ',100')"
                            />
    ...
    <xsl:template match="place">
        <Placemark>
            <name>
                <xsl:value-of select="current()//placeName[@type = 'short']"/>
            </name>
            <description>
                <xsl:value-of select="current()//placeName[@type = 'full']"/>
            </description>
            ...
            <Point>
                <coordinates>
                    <xsl:value-of
                        select="
                            current()/location/concat(string-join(
                            (substring-after(geo, ','),
                            substring-before(geo, ','))
                            , ','), ',100')"
                    />
